...
class ComfortModel:
    ...
    @classmethod
    def from_bayesian_csv(
        cls,
        csv_path: Path,
        apartment_id: str,
        rho: float = 0.0,
        fallback_to_population: bool = True,
    ) -> "ComfortModel":
        """
        Build a per-apartment ComfortModel directly and exclusively from
        Phase 3's bayesian_preference_results.csv (one row per apartment:
        T_star, RH_star, sigma_T, sigma_RH).

        The CSV has no T-RH correlation term, so rho defaults to 0.0
        (independence assumed) rather than borrowing the population-level
        rho=0.095 from Bayesian Run C — this keeps the comfort model built
        entirely from the CSV, with no values mixed in from another
        source/run. Pass rho explicitly if a non-zero correlation is
        later estimated per apartment.

        If apartment_id is missing from the file (e.g. AP41/AP42 are
        currently absent) and fallback_to_population=True, the mean over
        all valid_for_reward=1 rows in this same CSV is used instead of
        raising (still CSV-only, no external numbers).
        """
        import csv as csv_module

        rows = list(csv_module.DictReader(Path(csv_path).open(encoding="utf-8")))
        match = next((r for r in rows if r["apartment_id"] == apartment_id), None)

        if match is None:
            if not fallback_to_population:
                raise KeyError(
                    f"No bayesian preference row for apartment_id={apartment_id!r}."
                )
            valid = [r for r in rows if r.get("valid_for_reward") == "1"]
            if not valid:
                raise ValueError("No valid rows available for population fallback.")
            t_star = sum(float(r["T_star"]) for r in valid) / len(valid)
            rh_star = sum(float(r["RH_star"]) for r in valid) / len(valid)
            sigma_t = sum(float(r["sigma_T"]) for r in valid) / len(valid)
            sigma_rh = sum(float(r["sigma_RH"]) for r in valid) / len(valid)
        else:
            t_star = float(match["T_star"])
            rh_star = float(match["RH_star"])
            sigma_t = float(match["sigma_T"])
            sigma_rh = float(match["sigma_RH"])

        mu = np.array([t_star, rh_star])
        sigma_mat = np.array([
            [sigma_t ** 2,              rho * sigma_t * sigma_rh],
            [rho * sigma_t * sigma_rh,  sigma_rh ** 2],
        ])
        sigma_inv = np.linalg.inv(sigma_mat)
        HI_star = nws_heat_index_celsius(t_star, rh_star)
        return cls(mu=mu, sigma_mat=sigma_mat, sigma_inv=sigma_inv, HI_star=HI_star)


DEFAULT_BAYESIAN_CSV: Path = Path(__file__).resolve().parent / "bayesian_preference_results.csv"


def _resolve_bayesian_csv(csv_path: Path | str | None = None) -> Path:
    """Resolve the Bayesian comfort CSV from common workspace locations."""
    if csv_path is None:
        csv_path = DEFAULT_BAYESIAN_CSV
    ...
    for root in search_roots:
        for possible in [root / candidate, root / candidate.name]:
            if possible.exists():
                return possible

    workspace_root = Path(__file__).resolve().parent.parent
    matches = list(workspace_root.rglob(candidate.name))
    if matches:
        return matches[0]

    raise FileNotFoundError(
        f"bayesian_preference_results.csv not found at {csv_path}. "
        "This loader only reads from the CSV — pass the correct csv_path."
    )
...
def load_comfort_model(
    ...
    """Load the comfort model for one apartment exclusively from
    bayesian_preference_results.csv (Phase 3 output).

    No JSON file and no hard-coded population posterior ("Run C") is
    used here by default — every value comes from the CSV, per
    apartment, with only the CSV's own population mean as a fallback for
    apartments currently missing a row (see ComfortModel.from_bayesian_csv).

    ComfortModel.from_json() and ComfortModel.from_bayesian_run_c() are
    still available directly on the class if you explicitly want either
    of those alternate sources.
    """
    resolved_path = _resolve_bayesian_csv(csv_path)
    print(f"  [comfort] Loaded apartment {apartment_id} from {resolved_path}")
    return ComfortModel.from_bayesian_csv(resolved_path, apartment_id)
...
if __name__ == "__main__":
    ...
    comfort_model = load_comfort_model(
        apartment_id="AP10",
        csv_path=Path("bayesian_preference_results.csv"),
    )
